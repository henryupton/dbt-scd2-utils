{#
  Generates SQL for the initial load of an SCD Type 0 table.

  SCD Type 0 keeps exactly one row per business key and never updates it: the
  original (first-seen) value is retained forever. The source is deduplicated to
  the earliest row per key (by updated_at ascending), and the standard audit
  columns are attached as constants so the table shares the same signature as the
  other SCD types:
...
{% macro get_initial_load_scd0_sql(arg_dict) %}
...
select
    ...
    {# SCD0 audit columns: a single, always-current, never-updated version per key. #}
