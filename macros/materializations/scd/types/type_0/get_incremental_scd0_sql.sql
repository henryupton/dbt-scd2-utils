{#
  Generates the SQL for incremental SCD Type 0 processing using a MERGE statement.

  SCD Type 0 keeps a single, immutable row per business key: the original value is
  retained and never updated. The source is deduplicated to the earliest row per
  key and merged on the business key alone, with no `when matched` clause — so
  existing keys are left untouched and only brand-new keys are inserted.

    - matched     -> no action (original value retained).
    - not matched -> insert a new row with the SCD0 audit columns.
  ...
  Returns:
    Complete MERGE SQL statement for SCD Type 0 incremental processing
#}

{% macro get_incremental_scd0_sql(arg_dict) %}
...
using (
    ...
    {# One row per key: keep the original (earliest) version by updated_at. #}
    dedup as (
        select *
        from source_data
        qualify row_number() over (partition by {{ unique_keys_csv }} order by {{ updated_at_col }} asc) = 1
    )
...
{# Type 0 never updates: existing keys are retained as-is, only new keys insert. #}
